...
public class ExtensionAnalyzationTest extends AbstractXtendTest {
	...
	public void testNonStaticJavaExtension() {
		final ExtensionFile file = parse("String nonStaticHelper(String str) :JAVA " + Helper.class.getName() + ".nonStaticHelper(java.lang.String) ; \n");
	...
	public void testNonStaticJavaExtensionWithSlot() {
		final ExtensionFile file = parse("String nonStaticHelperWithSlot(String str) :JAVA [instanceSlot]" + Helper.class.getName() + ".nonStaticHelper(java.lang.String) ; \n");
		file.analyze(ec, issues);
		dumpIssues();
	...
	public void testNonExistentJavaMethod() {
		final ExtensionFile file = parse("String nonExistentMethod(String str) :JAVA " + Helper.class.getName() + ".nonExistentMethod(java.lang.String) ; \n");
		file.analyze(ec, issues);
		dumpIssues();
		assertEquals(1, issues.size());
		assertEquals(AnalysationIssue.Type.FEATURE_NOT_FOUND, issues.iterator().next().getType());
	...
	public final void testExpressionExtension() {
        final ExtensionFile file = parse("String plainExtension() : 'zzz' ;\n"
        ...
        final ExpressionExtensionStatement ext = (ExpressionExtensionStatement) file.getExtensions().get(1);
	...
	public final void testAmbigous() {
		...
		final ExtensionFile file = parse("import '" + aType.getMetaType().getEPackage().getName() + "';" + "doStuff(" + aType.getMetaType().getName() + " this) : true; " + "doStuff(" + aType.getMetaTypeC().getName() + " this) : false;" + "bla(" + aType.getMetaTypeB().getName()
				+ " this) : this.doStuff();");

		ExecutionContext ctx = aType.newContext(file);
		// XXX can't use file.analyze unless metamodel registration works
		// ExtensionFile.analyze clones context and getAllVisibleMetamodel
		// override goes away
