 ...
 * @generated
 */
public class AffixednodeInitDiagramFileAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		try {
			...
			AffixednodeDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + domainModelURI, ex); //$NON-NLS-1$
		}
		if (diagramRoot == null) {
		...
		Wizard wizard = new AffixednodeNewDiagramFileWizard(domainModelURI, diagramRoot, editingDomain);
		wizard.setWindowTitle(NLS.bind(Messages.InitDiagramFile_WizardTitle, CanvasEditPart.MODEL_ID));
		AffixednodeDiagramEditorUtil.runWizard(getShell(), wizard, "InitDiagramFile"); //$NON-NLS-1$
	}
}
