...
import org.eclipse.gmf.tooling.examples.compartments.diagram.part.CompartmentsDiagramUpdater;
import org.eclipse.gmf.tooling.examples.compartments.diagram.part.CompartmentsLinkDescriptor;
import org.eclipse.gmf.tooling.examples.compartments.diagram.part.CompartmentsNodeDescriptor;
...
public class CanvasCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		List<CompartmentsNodeDescriptor> childDescriptors = CompartmentsDiagramUpdater.getCanvas_1000SemanticChildren(viewObject);
		for (CompartmentsNodeDescriptor d : childDescriptors) {
	...
	protected void refreshSemantic() {
		...
		List<CompartmentsNodeDescriptor> childDescriptors = CompartmentsDiagramUpdater.getCanvas_1000SemanticChildren((View) getHost().getModel());
		...
		for (Iterator<CompartmentsNodeDescriptor> descriptorsIterator = childDescriptors.iterator(); descriptorsIterator.hasNext();) {
			CompartmentsNodeDescriptor next = descriptorsIterator.next();
			String hint = CompartmentsVisualIDRegistry.getType(next.getVisualID());
		...
		for (CompartmentsNodeDescriptor next : childDescriptors) {
			String hint = CompartmentsVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator.hasNext();) {
			...
			int diagramLinkVisualID = CompartmentsVisualIDRegistry.getVisualID(nextDiagramLink);
			...
			for (Iterator<CompartmentsLinkDescriptor> linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator.hasNext();) {
				CompartmentsLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator.next();
	...
	private Collection<CompartmentsLinkDescriptor> collectAllLinks(View view, Domain2Notation domain2NotationMap) {
		...
		case CanvasEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getCanvas_1000ContainedLinks(view));
			}
		...
		case TopNodeAEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getTopNodeA_2001ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case TopNodeBEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getTopNodeB_2002ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case ChildOfA_CEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getChildOfA_C_3001ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case ChildOfA_DEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getChildOfA_D_3002ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case ChildOfB_EEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getChildOfB_E_3003ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case ChildOfB_GEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getChildOfB_G_3004ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case ChildOfB_FEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CompartmentsDiagramUpdater.getChildOfB_F_3006ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	private Collection<IAdaptable> createConnections(Collection<CompartmentsLinkDescriptor> linkDescriptors, Domain2Notation domain2NotationMap) {
		...
		for (CompartmentsLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(nextLinkDescriptor.getSemanticAdapter(),
					CompartmentsVisualIDRegistry.getType(nextLinkDescriptor.getVisualID()), ViewUtil.APPEND, false, ((IGraphicalEditPart) getHost()).getDiagramPreferencesHint());
