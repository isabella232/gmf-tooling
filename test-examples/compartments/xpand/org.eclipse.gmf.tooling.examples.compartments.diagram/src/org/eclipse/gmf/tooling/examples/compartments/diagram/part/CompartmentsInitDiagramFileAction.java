 ...
 * @generated
 */
public class CompartmentsInitDiagramFileAction implements IObjectActionDelegate {
	...
	*/
	public void run(IAction action) {
		...
		try {
			...
			CompartmentsDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + domainModelURI, ex); //$NON-NLS-1$
		}
		if (diagramRoot == null) {
		...
		Wizard wizard = new CompartmentsNewDiagramFileWizard(domainModelURI, diagramRoot, editingDomain);
		wizard.setWindowTitle(NLS.bind(Messages.InitDiagramFile_WizardTitle, CanvasEditPart.MODEL_ID));
		CompartmentsDiagramEditorUtil.runWizard(getShell(), wizard, "InitDiagramFile"); //$NON-NLS-1$
	}
}
