 ...
 * @generated
 */
public class ShortcutInitDiagramFileAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		try {
			...
			ShortcutDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + domainModelURI, ex); //$NON-NLS-1$
		}
		if (diagramRoot == null) {
		...
		Wizard wizard = new ShortcutNewDiagramFileWizard(domainModelURI, diagramRoot, editingDomain);
		wizard.setWindowTitle(NLS.bind(Messages.InitDiagramFile_WizardTitle, DiagramEditPart.MODEL_ID));
		ShortcutDiagramEditorUtil.runWizard(getShell(), wizard, "InitDiagramFile"); //$NON-NLS-1$
	}
}
