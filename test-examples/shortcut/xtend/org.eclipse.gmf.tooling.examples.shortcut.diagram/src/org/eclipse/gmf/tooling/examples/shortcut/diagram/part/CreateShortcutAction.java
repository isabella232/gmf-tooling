...
public class CreateShortcutAction extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		ShortcutElementChooserDialog elementChooser = new ShortcutElementChooserDialog(shell, view);
		...
		try {
			...
			ShortcutDiagramEditorPlugin.getInstance().logError("Exception while loading object: " + selectedModelElementURI.toString(), e); //$NON-NLS-1$
			return null;
		}
		...
		command = command.compose(new ShortcutCreateShortcutDecorationsCommand(editingDomain, view, viewDescriptor));
		try {
			OperationHistoryFactory.getOperationHistory().execute(command, new NullProgressMonitor(), null);
		} catch (ExecutionException e) {
			ShortcutDiagramEditorPlugin.getInstance().logError("Unable to create shortcut", e); //$NON-NLS-1$
		}
		return null;
	}
}
