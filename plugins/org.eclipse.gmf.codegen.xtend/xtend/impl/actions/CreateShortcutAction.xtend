...
class CreateShortcutAction {
	...
	def executeMethod(org.eclipse.gmf.codegen.gmfgen.CreateShortcutAction it) '''
		...
		public Object execute(org.eclipse.core.commands.ExecutionEvent event) throws org.eclipse.core.commands.ExecutionException {
			...
			«_assert('((org.eclipse.jface.viewers.IStructuredSelection) selection).getFirstElement() instanceof org.eclipse.gef.EditPart')»
			...
			try {
				...
				«owner.editorGen.plugin.getActivatorQualifiedClassName()».getInstance().logError("Exception while loading object: " + selectedModelElementURI.toString(), e); «nonNLS(1)»
				return null;
			}
			...
			try {
				...
				«owner.editorGen.plugin.getActivatorQualifiedClassName()».getInstance().logError("Unable to create shortcut", e); «nonNLS(1)»
			}
			«ELSE»
			org.eclipse.emf.ecore.resource.Resource resource = «owner.editorGen.diagram.getDiagramEditorUtilQualifiedClassName()».openModel(shell, «xptExternalizer.accessorCall(owner.editorGen, titleKey(i18nKeyForCreateShortcutOpenModel()))», editingDomain);
			if (resource == null || resource.getContents().isEmpty()) {
				return null;
			}
			«xptShortcutCreationWizard.qualifiedClassName(owner.editorGen.diagram)» wizard = new «xptShortcutCreationWizard.qualifiedClassName(owner.editorGen.diagram)»((org.eclipse.emf.ecore.EObject) resource.getContents().get(0), view, editingDomain);
			wizard.setWindowTitle(«xptExternalizer.accessorCall(it.owner.editorGen, titleKey(i18nKeyForCreateShortcutWizard()))»);
			«owner.editorGen.diagram.getDiagramEditorUtilQualifiedClassName()».runWizard(myShell, wizard, "CreateShortcut"); «nonNLS(1)»
			«ENDIF»
			return null;
		}
	'''
