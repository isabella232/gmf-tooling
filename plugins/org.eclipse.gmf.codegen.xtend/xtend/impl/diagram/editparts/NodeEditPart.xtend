...
'''
	...
	«FOR label : getInnerFixedLabels(it)»
		...
		if (childEditPart instanceof «label.getEditPartQualifiedClassName()») {
			((«label.editPartQualifiedClassName») childEditPart).«xptTextAware.labelSetterName(childViewmap)»(getPrimaryShape().«childViewmap.getterName»());
			return true;
		}
	«ENDFOR»
	«FOR label : getInnerFixedLabelsWithModeledViewmaps(it)»
		...
		if (childEditPart instanceof «label.getEditPartQualifiedClassName()») {
			((«label.getEditPartQualifiedClassName()») childEditPart).«xptTextAware.labelSetterName(childViewmap)»(getPrimaryShape().«getterName»());
			return true;
		}
	«ENDFOR»
...
'''
	...
	«IF !getExternalLabels(it).empty»
		...
		protected void addBorderItem(org.eclipse.draw2d.IFigure borderItemContainer, org.eclipse.gmf.runtime.diagram.ui.editparts.IBorderItemEditPart borderItemEditPart) {
			if («FOR label : getExternalLabels(it) SEPARATOR ' || '»borderItemEditPart instanceof «label.editPartQualifiedClassName»«ENDFOR») {
	...
	@MetaDef def newArrayListOfElementTypes(String varName) '''java.util.ArrayList<org.eclipse.gmf.runtime.emf.type.core.IElementType> «varName» = new java.util.ArrayList<org.eclipse.gmf.runtime.emf.type.core.IElementType>'''
	@MetaDef def newLinkedListOfElementTypes(String varName) '''java.util.LinkedList<org.eclipse.gmf.runtime.emf.type.core.IElementType> «varName» = new java.util.LinkedList<org.eclipse.gmf.runtime.emf.type.core.IElementType>'''

	// pre: getAssistantOutgoingLinks(this).size() > 0
	def getMARelTypesOnSource(GenNode it) '''
		«generatedMemberComment»
	...
	def getMARelTypesOnTarget(GenNode it) '''
		«generatedMemberComment»
	...
	// pre: getAssistantOutgoingLinks(this).size() > 0
	def getMARelTypesOnSourceAndTarget(GenNode it) '''
		«generatedMemberComment»
	...
	// pre: getAssistantIncomingLinks(this).size() > 0
	def getMATypesForSource(GenNode it) '''
		«generatedMemberComment»
	...
	// pre: getAssistantOutgoingLinks(this).size() > 0
	def getMATypesForTarget(GenNode it) '''
		«generatedMemberComment»
