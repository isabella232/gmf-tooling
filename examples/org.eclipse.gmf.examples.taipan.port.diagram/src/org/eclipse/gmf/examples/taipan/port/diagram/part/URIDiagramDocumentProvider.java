...
public class URIDiagramDocumentProvider extends AbstractDocumentProvider implements IDiagramDocumentProvider {
	...
	protected ElementInfo createElementInfo(Object element) throws CoreException {
		if (element instanceof URIEditorInput) {
			...
			try {
				...
				PortDiagramEditorPlugin.getInstance().logError("Error creating document from uri", x);
				status = x.getStatus();
	...
	protected boolean setDocumentContent(IDocument document, IEditorInput editorInput) throws CoreException {
		if (editorInput instanceof org.eclipse.gmf.examples.taipan.port.diagram.part.URIEditorInputProxy) {
			org.eclipse.gmf.examples.taipan.port.diagram.part.URIEditorInputProxy diagramElement = (org.eclipse.gmf.examples.taipan.port.diagram.part.URIEditorInputProxy) editorInput;
	...
	protected void setDocumentContentFromStorage(IDocument document, org.eclipse.emf.common.util.URI uri) throws CoreException {
		...
		try {
			...
			if (e instanceof CoreException) {
				...
				thrownExcp = new CoreException(new Status(IStatus.ERROR, PortDiagramEditorPlugin.ID, 0, msg != null ? msg : "Error loading diagram", e)); //$NON-NLS-1$
			}
			throw thrownExcp;
		}
	}
			...
			for (Iterator it = resources.iterator(); it.hasNext();) {
				...
				if (nextResource.isLoaded()) {
					try {
						...
						PortDiagramEditorPlugin.getInstance().logError("Unable to save resource: " + nextResource.getURI(), e);
					}
				}
				monitor.worked(1);
			}
