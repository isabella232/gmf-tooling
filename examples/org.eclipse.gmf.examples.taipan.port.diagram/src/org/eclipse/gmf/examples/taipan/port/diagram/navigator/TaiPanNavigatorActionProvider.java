...
import org.eclipse.gmf.examples.taipan.port.diagram.part.TaiPanDiagramEditor;
	...
	private class OpenDiagramAction extends Action {
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof Diagram) {
					...
					if (PortEditPart.MODEL_ID.equals(TaiPanVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		public void run() {
			...
			try {
				...
				PortDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e);
			}
		}
