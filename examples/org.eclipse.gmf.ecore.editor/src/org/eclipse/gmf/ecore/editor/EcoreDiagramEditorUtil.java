...
public class EcoreDiagramEditorUtil extends IDEEditorUtil {
	...
	public static final IFile createNewDiagramFile(DiagramFileCreator diagramFileCreator, IPath containerFullPath, String fileName, InputStream initialContents, String kind, Shell shell,
		...
		modelFilePath = modelFilePath.addFileExtension("ecore"); //$NON-NLS-1$
		...
		try {
			...
			IStatus status = new Status(IStatus.ERROR, EcoreDiagramEditorPlugin.ID, 0, e.getMessage(), e);
			EcoreDiagramEditorPlugin.getInstance().getLog().log(status);
		}
		...
		if (notationModel != null) {
			...
			OperationUtil.runAsUnchecked(new MRunnable() {
				...
				public Object run() {
					EObject model = EObjectUtil.create(EcorePackage.eINSTANCE.getEPackage());
					Diagram diagram = ViewService.createDiagram(model, kindParam, new PreferencesHint(EcoreDiagramEditorPlugin.EDITOR_ID));
					if (diagram != null) {
