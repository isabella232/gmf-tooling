...
import org.eclipse.gmf.examples.mindmap.lite.rcp.diagram.part.MindmapCreationWizard;
import org.eclipse.gmf.examples.mindmap.lite.rcp.diagram.part.MindmapDiagramEditorPlugin;
import org.eclipse.gmf.examples.mindmap.lite.rcp.diagram.part.MindmapDiagramEditorUtil;
			...
			if (Dialog.OK == loadResourceDialog.open()) {
				for (Iterator it = loadResourceDialog.getURIs().iterator(); it
					...
					MindmapDiagramEditorUtil
							.openEditor((org.eclipse.emf.common.util.URI) it
									.next());
	...
	public static class OpenAction extends WorkbenchWindowActionDelegate {
		...
		public void run(IAction action) {
			String filePath = MindmapDiagramEditorUtil.openFilePathDialog(
					getWindow().getShell(), "*.mmd", org.eclipse.swt.SWT.OPEN); //$NON-NLS-1$
			if (filePath != null) {
				MindmapDiagramEditorUtil
						.openEditor(org.eclipse.emf.common.util.URI
