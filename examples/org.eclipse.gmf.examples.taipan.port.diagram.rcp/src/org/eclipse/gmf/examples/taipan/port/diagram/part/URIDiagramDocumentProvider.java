	...
	protected ElementInfo createElementInfo(Object element) throws CoreException {
		if (element instanceof URIEditorInput) {
			...
			try {
				...
				TaiPanDiagramEditorPlugin.getInstance().logError("Error creating document from uri", x);
				status = x.getStatus();
			...
			for (Iterator it = resources.iterator(); it.hasNext();) {
				...
				if (nextResource.isLoaded()) {
					try {
						...
						TaiPanDiagramEditorPlugin.getInstance().logError("Unable to save resource: " + nextResource.getURI(), e);
					}
				}
				monitor.worked(1);
			}
