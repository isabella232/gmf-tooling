...
public class URIDiagramDocumentProvider extends AbstractDocumentProvider
		...
		if (element instanceof URIEditorInput) {
			...
			try {
				...
				MindmapDiagramEditorPlugin.getInstance().logError(
						"Error creating document from uri", x);
				status = x.getStatus();
	...
	public ElementInfo createNewElementInfo(IDocument document) {
		DiagramModificationListener listener = new DiagramModificationListener(
	...
	protected boolean setDocumentContent(IDocument document,
		...
		if (editorInput instanceof URIEditorInput) {
			setDocumentContentFromStorage(document,
	 ...
	 * @generated
	 */
	protected void setDocumentContentFromStorage(IDocument document, URI uri)
			throws CoreException {
		...
		try {
			...
			if (e instanceof CoreException) {
				...
				thrownExcp = new CoreException(new Status(IStatus.ERROR,
						MindmapDiagramEditorPlugin.ID, 0, msg != null ? msg
