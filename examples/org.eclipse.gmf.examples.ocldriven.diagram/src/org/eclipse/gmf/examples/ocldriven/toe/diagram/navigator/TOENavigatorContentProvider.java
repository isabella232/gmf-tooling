 ...
 * @generated
 */
public class TOENavigatorContentProvider implements ICommonContentProvider {
	...
	*/
	...
	public TOENavigatorContentProvider() {
	...
	*/
	public Object[] getChildren(Object parentElement) {
		if (parentElement instanceof IFile) {
			...
			ArrayList<TOENavigatorItem> result = new ArrayList<TOENavigatorItem>();
		...
		if (parentElement instanceof TOENavigatorGroup) {
			TOENavigatorGroup group = (TOENavigatorGroup) parentElement;
			return group.getChildren();
		}

		if (parentElement instanceof TOENavigatorItem) {
			TOENavigatorItem navigatorItem = (TOENavigatorItem) parentElement;
	...
	*/
	private Object[] getViewChildren(View view, Object parentElement) {
		...
		case ContributionEditPart.VISUAL_ID: {
			...
			Edge sv = (Edge) view;
			TOENavigatorGroup target = new TOENavigatorGroup(Messages.NavigatorGroupName_Contribution_4003_target, "icons/linkTargetNavigatorGroup.gif", parentElement); //$NON-NLS-1$
			TOENavigatorGroup source = new TOENavigatorGroup(Messages.NavigatorGroupName_Contribution_4003_source, "icons/linkSourceNavigatorGroup.gif", parentElement); //$NON-NLS-1$
			...
			connectedViews = getLinksSourceByType(Collections.singleton(sv), TOEVisualIDRegistry.getType(EmployeeEditPart.VISUAL_ID));
			...
			source.addChildren(createNavigatorItems(connectedViews, source, true));
			connectedViews = getLinksSourceByType(Collections.singleton(sv), TOEVisualIDRegistry.getType(Employee2EditPart.VISUAL_ID));
			source.addChildren(createNavigatorItems(connectedViews, source, true));
			if (!target.isEmpty()) {
				result.add(target);
			}
	...
	*/
	 ...
	 * @generated
	 */
	private Collection<TOENavigatorItem> createNavigatorItems(Collection<View> views, Object parent, boolean isLeafs) {
		ArrayList<TOENavigatorItem> result = new ArrayList<TOENavigatorItem>(views.size());
		for (View nextView : views) {
			result.add(new TOENavigatorItem(nextView, parent, isLeafs));
		}
		return result;
	}
	...
	*/
	public Object getParent(Object element) {
		if (element instanceof TOEAbstractNavigatorItem) {
			TOEAbstractNavigatorItem abstractNavigatorItem = (TOEAbstractNavigatorItem) element;
