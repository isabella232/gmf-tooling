 ...
 * @generated
 */
public class DesignDocumentProvider extends FileDiagramDocumentProvider {
		...
		for (Iterator it = resources.iterator(); it.hasNext();) {
			...
			if (nextResource != diagramResource && nextResource.isLoaded()) {
				try {
					...
					DesignDiagramEditorPlugin.getInstance().logError("Unable to save resource: " + nextResource.getURI(), e); //$NON-NLS-1$
				}
			}
			monitor.worked(1);
		}
	...
	private class CustomModificationListener extends FileDiagramModificationListener {
		 ...
		 * @generated
		 */
		public CustomModificationListener(DesignDocumentProvider documentProviderParameter, DiagramDocument documentParameter, IFileEditorInput inputParameter) {
