 ...
 * @generated
 */
public class DesignNewDiagramFileWizard extends Wizard {
	...
	public boolean performFinish() {
		...
		try {
			...
			DesignDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramFile.getFullPath().toString(), ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			DesignDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
